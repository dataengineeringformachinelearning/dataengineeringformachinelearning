...
**Identify Attack Vectors.** Document paths an adversary could traverse to reach target assets, including multiple techniques per pathway. Categorize threats (e.g., STRIDE-LM) and incorporate current threat intelligence. *DEML example:* Enumerated vectors include JWT forgery against Django REST, cross-tenant IDOR via predictable IDs (mitigated by UUID PKs), broker poisoning on `frontend-events`, model inversion against `/api/v1/predict`, and credential stuffing against Firebase Auth. Attack trees for the ingest path note that a compromised integration key allows arbitrary event injection until ABAC and rate limits (`deml-dragonfly`) throttle the source. *Pipeline-builder example:* A customer's detection pipeline faces vectors such as training-data poisoning (malicious rows in ingest batches), label-flip attacks on feedback loops, and replay of captured ingest payloads—each mapped to a specific hop in their DFD.
...
| **I** — Identify attack vectors | UUID PK anti-IDOR, broker/DLQ failure modes [§13](#13-contingency--degraded-operations), network enrichment ([Chapter 20](#chapter-20-network-traffic-enrichment-and-cybersecurity-telemetry)) |
