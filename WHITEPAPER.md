...
Additionally, the platform implements a strict 30-day hot-retention lifecycle policy for raw telemetry data. All raw telemetry endpoint data is purged from the hot PostgreSQL database exactly 30 days after creation. Long-term raw metrics are routed to ClickHouse for OLAP querying. High-value business objects, such as incident histories, bug reports, and threat reports, are maintained indefinitely as the system of record. The ML training worker automatically triggers full model retraining and telemetry cleanup upon application deployment and runs continuously every day.
