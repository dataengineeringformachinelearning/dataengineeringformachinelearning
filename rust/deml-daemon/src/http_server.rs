...
async fn ingest(
    ...
    let inserted = sqlx::query_scalar::<_, Uuid>(
        r#"
        INSERT INTO monitor_outboxevent
    ...
    .bind(event)
    .bind(json!({"version": "1.0", "event_type": "ingestion", "source": "deml-rust-ingest"}))
...
async fn authenticate(
    ...
    let row = sqlx::query_as::<_, (String, Uuid, String)>(
        r#"
        SELECT key.key_hash, profile.account_id, profile.tier
        FROM monitor_apikey AS key
        JOIN user_profiles AS profile ON profile.user_id = key.user_id
        WHERE key.prefix = $1 AND key.is_active = true
        "#,
    )
    .bind(prefix)
    .fetch_optional(pool)
