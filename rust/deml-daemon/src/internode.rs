...
const ENVELOPE_TYPE: &str = "deml-internode+jwe";
...
fn load_keyring() -> Result<Keyring> {
    ...
    let mode = match env::var("DEML_INTERNODE_ENCRYPTION")
        ...
        .as_str()
    {
        ...
        _ => bail!("DEML_INTERNODE_ENCRYPTION must be disabled, optional, or required"),
    };
    let sender = env::var("DEML_NODE_ID")
        .or_else(|_| env::var("RAILWAY_SERVICE_NAME"))
        .unwrap_or_else(|_| "deml-local".to_string());
    validate_identifier("DEML_NODE_ID", &sender)?;
    let active_kid = env::var("DEML_INTERNODE_ACTIVE_KID")
        .ok()
        .filter(|value| !value.trim().is_empty());
    let raw_keys = env::var("DEML_INTERNODE_KEYS").unwrap_or_else(|_| "{}".to_string());
    let encoded_keys: HashMap<String, String> =
        serde_json::from_str(&raw_keys).context("DEML_INTERNODE_KEYS must be a JSON object")?;
    ...
    if let Some(kid) = active_kid.as_deref() {
        validate_identifier("DEML_INTERNODE_ACTIVE_KID", kid)?;
        if !keys.contains_key(kid) {
            bail!("DEML_INTERNODE_ACTIVE_KID is not present in DEML_INTERNODE_KEYS");
        }
    }
    if mode == Mode::Required && active_kid.is_none() {
        bail!("required internode encryption needs DEML_INTERNODE_ACTIVE_KID");
    }
    Ok(Keyring {
        mode,
        active_kid,
...
pub fn encrypt_kafka_value(value: &[u8], topic: &str) -> Result<Vec<u8>> {
...
pub fn decrypt_kafka_value(value: &[u8], topic: &str) -> Result<Vec<u8>> {
    decrypt_with(
        value,
        &format!("kafka:{topic}"),
        &load_keyring()?,
...
mod tests {
    ...
    fn round_trips_and_rejects_context_swaps() {
        ...
        .unwrap();
        let envelope: Envelope = serde_json::from_slice(&encrypted).unwrap();
        assert_eq!(
            envelope.ciphertext,
            "PCCifquErG-ve7W7gdlIXbPmtxnAS29MFVfVtS1EMIIxIIPMn_\
             EiqESUT924txkak-sNbr-oICt4Caj8XnDvij8"
